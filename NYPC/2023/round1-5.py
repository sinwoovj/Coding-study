...
def operate_team(dtm, optm):
    ...
    for pl in optm:
        ...
        if df == 0: optm = [d for d in optm if d["pos"] != "DF"]
...
for player in sorted_unique_data:
    if player["pos"] == "GK":
        ...
        FW.append(player)
sorted_GK = sorted(GK, key=lambda x: (-int(x["over"]),int(x["cost"]),int(x["salary"])))
sorted_DF = sorted(DF, key=lambda x: (-int(x["over"]),int(x["cost"]),int(x["salary"])))
sorted_MF = sorted(MF, key=lambda x: (-int(x["over"]),int(x["cost"]),int(x["salary"])))
sorted_FW = sorted(FW, key=lambda x: (-int(x["over"]),int(x["cost"]),int(x["salary"])))
